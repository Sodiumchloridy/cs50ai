...
def shortest_path(source, target):
    ...
    """
    # Track numbers of state explored
    explored_num = 0

    # Initialize frontier to the source
    head = Node(state=source, parent=None, action=None)
    ...
    # Runs loop until solution is found
    while True:
        ...
        for movie_id, person_id in neighbors_for_person(curr.state):
            if not frontier.contains_state(person_id) and person_id not in explored:
                child = Node(state=person_id, parent=curr, action=movie_id)
